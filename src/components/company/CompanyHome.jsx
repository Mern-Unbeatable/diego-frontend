...
const rows = [
    { id: 1, course: 'Formazione generale', student: 'Franco Rossi', status: 'completato', progress: 100, date: 'GG/MM/AAAA', action: 'download' },
    { id: 2, course: 'Formazione rischio medio', student: 'Franco Rossi', status: 'in corso', progress: 75, date: 'GG/MM/AAAA', action: 'reminder' },
    { id: 3, course: 'Formazione Seveso', student: 'Franco Rossi', status: 'in corso', progress: 25, date: 'GG/MM/AAAA', action: 'reminder' },
    { id: 4, course: 'Aggiornamento RSPP', student: 'Franco Rossi', status: 'non iniziato', progress: 0, date: 'GG/MM/AAAA', action: 'reminder' },
];
...
const CompanyHome = () => {
    return (
        <section className="space-y-7">
            ...
            <div>
                <h3 className="mb-4 text-[24px] font-semibold text-[#202020]">Panoramica</h3>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {overviewCards.map((card) => (
                        <article key={card.title} className="rounded-xl border border-[#ececec] bg-white p-5 shadow-[0_1px_0_#f3f3f3]">
                            <div className="mb-3 flex justify-center">
                                <div className={`flex h-9 w-9 items-center justify-center rounded-md ${card.iconBg} text-white`}>
                                    {card.icon}
                                </div>
                            </div>
                            <p className="text-center text-[15px] font-semibold text-[#2d2d2d]">{card.title}</p>
                            <p className={`mt-1 text-center text-sm font-medium ${card.textColor}`}>{card.value}</p>
                        </article>
                    ))}

                    <article className="rounded-xl border border-[#ececec] bg-white p-5 shadow-[0_1px_0_#f3f3f3]">
                        <p className="text-center text-[30px] font-semibold text-[#202020]">Acquista nuovi corsi</p>
            ...
            <section className="overflow-hidden rounded-xl border border-[#e8e8e8] bg-white">
                <header className="border-b border-[#ececec] px-5 py-5">
                    <h4 className="text-[30px] font-semibold text-[#1f1f1f]">Stato di avanzamento - Seveso Training</h4>
                </header>

                <div className="grid grid-cols-1 gap-3 border-b border-[#ececec] px-5 py-4 lg:grid-cols-4">
                    <div>
                        <p className="mb-1 text-xs font-medium text-[#868686]">Tutte le aziende</p>
                        <input className="h-10 w-full rounded-full border border-[#e5e5e5] px-4 text-sm outline-none placeholder:text-[#a3a3a3]" placeholder="Cerca per nome" />
                    </div>
                    <div>
                    ...
                    <div>
                        <p className="mb-1 text-xs font-medium text-[#868686]">Cerca partecipante</p>
                        <select className="h-10 w-full rounded-full border border-[#e5e5e5] px-4 text-sm text-[#555555] outline-none">
                            ...
                            {rows.map((row) => (
                                <tr key={row.id} className="border-b border-[#f0f0f0] text-sm">
                                    ...
                                    <td className="px-3 py-3">
                                        <span className={`rounded-full px-2 py-1 text-[11px] font-semibold ${statusTone[row.status]}`}>
                                            {row.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-3 py-3">
                                        {row.action === 'download' ? (
                                            <button type="button" className="inline-flex items-center gap-2 rounded-full bg-[#73bfa1] px-4 py-1.5 text-xs font-semibold text-white hover:bg-[#63a88c]">
                                                <Download size={13} /> Download
                                            </button>
                                        ) : (
                                            <button type="button" className="inline-flex items-center gap-2 rounded-full bg-[#e6f6ef] px-4 py-1.5 text-xs font-semibold text-[#57a080] hover:bg-[#d9f1e7]">
                ...
                <footer className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 text-sm text-[#7d7d7d]">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button type="button" className="h-6 w-6 rounded bg-[#73bfa1] text-xs font-semibold text-white">1</button>
