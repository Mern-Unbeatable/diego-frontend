...
export default function CourseFormModal({
  ...
  return (
    ...
    >
      ...
      {isLoadingCourse ? (
        <Loading size="md" className="mt-6 sm:mt-10 min-h-40 sm:min-h-60" />
      ) : courseLoadError ? (
        <div className="mx-auto mt-6 sm:mt-10 max-w-xl rounded-2xl border border-[#f0d4cf] bg-[#fff7f5] p-4 sm:p-6 md:p-8 text-center">
          <p className="text-base sm:text-lg font-medium text-[#b42318]">Impossibile caricare il corso</p>
          <p className="mt-2 text-xs sm:text-sm text-[#7a4f47]">
            {isNetworkError
              ? 'Impossibile contattare il server API. Avvia il backend (porta 5000) e riavvia il frontend.'
              : isAuthError
                ? 'Sessione scaduta o non valida. Esci, accedi di nuovo e riprova.'
        ...
        <Form
          ...
          defaultValues={defaultValues}
          onSubmit={() => {}}
          className="mx-auto mt-6 sm:mt-10 w-full max-w-[720px] space-y-4"
        >
          <CourseForm
            savedCourseId={savedCourseId}
            onSaveCourse={handleSaveCourse}
