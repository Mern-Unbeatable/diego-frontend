...
const SupportFeedbackView = () => {
    ...
    return (
        <div className="min-h-screen bg-[#efefef] p-3 md:p-6">
            <div className="mx-auto max-w-[1320px] rounded-xl bg-[#f8f8f8] p-6 md:p-10">
                <div className="relative mb-8 flex items-center justify-center">
                    ...
                    <h1 className="text-[26px] font-semibold text-[#202020] md:text-[34px]">Area ticket</h1>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1.6fr_0.8fr]">
                    <div className="space-y-5">
                        <section className="rounded-md border border-[#e4e4e4] bg-white p-5 md:p-6">
                            <h2 className="mb-4 text-[20px] font-semibold text-[#262626] md:text-[30px]">Descrizione ticket</h2>
                            <p className="whitespace-pre-line text-[12px] leading-relaxed text-[#333] md:text-[16px]">{ticket.description}</p>
                        </section>

                        <section className="rounded-md border border-[#e4e4e4] bg-white p-5 md:p-6">
                            <h2 className="mb-2 text-[20px] font-semibold text-[#262626] md:text-[30px]">Conversazione</h2>
                                ...
                                {ticket.conversation.map((msg) => (
                                    <div key={msg.id} className="flex gap-3">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[13px] font-semibold text-[#222] md:text-[16px]">{msg.author}</p>
                                            <p className="mb-1 text-[10px] text-[#9a9a9a] md:text-[12px]">{msg.timestamp}</p>
                                            <div className="rounded-md bg-[#edf6f1] p-3">
                                                <p className="whitespace-pre-line text-[11px] text-[#2f2f2f] md:text-[14px]">{msg.message}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <section>
                            <h2 className="mb-3 text-[24px] font-semibold text-[#262626] md:text-[32px]">Rispondi</h2>
                            <textarea
                    ...
                    <aside>
                        <div className="rounded-md border border-[#e4e4e4] bg-white p-5 md:p-6">
                            <h2 className="mb-4 text-[20px] font-semibold text-[#262626] md:text-[30px]">Dettagli ticket</h2>
                            <div className="space-y-2 text-[12px] leading-relaxed text-[#2f2f2f] md:text-[16px]">
                                ...
                                <p>
                                    <span className="font-semibold">Tipologia richiesta:</span> {ticket.details.requestType}
                                </p>
                                <p>
                                    <span className="font-semibold">Priorita:</span> {ticket.details.priority}
                                </p>
                                <p>
                                    <span className="font-semibold">Creato il:</span> {ticket.details.createdAt}
                                </p>
                                <p>
                                    <span className="font-semibold">Nominativo:</span> {ticket.details.assignedTo}
                                </p>
                            </div>
                        </div>
                    </aside>
