...
import { CalendarDays, Mail, Pencil, Phone, Trash2, UsersRound } from 'lucide-react';
import EmployeeModal from './EmployeeModal';

const employeesSeed = [
    { id: 1, name: 'Franco rossi', role: 'Safety manager', email: 'willie.jennings@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Attivo' },
    { id: 2, name: 'Franco rossi', role: 'Safety manager', email: 'michelle.rivera@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Attivo' },
    { id: 3, name: 'Franco rossi', role: 'Safety manager', email: 'alma.lawson@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Attivo' },
    { id: 4, name: 'Franco rossi', role: 'Safety manager', email: 'curtis.weaver@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Attivo' },
    { id: 5, name: 'Franco rossi', role: 'Safety manager', email: 'kenzi.lawson@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Attivo' },
    { id: 6, name: 'Franco rossi', role: 'Safety manager', email: 'georgia.young@example.com', phone: '+39 123 456 7890', hireDate: '15/03/2022', status: 'Inattivo' },
];
...
const CompanyTrainingView = () => {
    ...
    const openEdit = (employee) => setModal({ open: true, mode: 'edit', employee });
    const closeModal = () => setModal({ open: false, mode: 'add', employee: null });
    ...
    return (
        <section className="space-y-6">
            <div className="flex items-center justify-between">
                <h2 className="text-2xl md:text-3xl font-semibold text-[#1f1f1f]">Anagrafica dipendenti</h2>
                <button
                    type="button"
                    onClick={openAdd}
            ...
            <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
                {visibleEmployees.map((employee) => (
                    <article key={employee.id} className="rounded-xl border border-[#e7e7e7] bg-white p-5">
                        <div className="mb-4 flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl md:text-2xl font-semibold text-[#1f1f1f]">{employee.name}</h3>
            ...
            <div className="rounded-xl border border-[#ececec] bg-white px-5 py-4">
                <p className="mb-3 text-sm text-[#666666]">Corso assegnato: Formazione SEVESO</p>
                <Link
            ...
            {modal.open ? (
                <EmployeeModal mode={modal.mode} employee={modal.employee} onClose={closeModal} />
            ) : null}
        </section>
    );
};
