...
function ProgressCell({ value }) {
    return (
        <div className="flex min-w-[220px] items-center gap-3">
            ...
            <span className="text-lg font-medium leading-none text-[#202020]">{String(value).padStart(2, '0')}</span>
        </div>
    )
}
...
function ActionPills({ type, onEdit }) {
    return (
        <div className="flex items-center gap-3">
            ...
            >
                {type === 'download' ? <Download size={16} strokeWidth={2.2} /> : <Send size={16} strokeWidth={2.2} />}
            </button>
        </div>
    )
}
        ...
        <section className="w-full bg-[#f6f6f6] p-6 md:p-8">
            <div className="mx-auto w-full">
                <div className="mb-11 flex items-center justify-between">
                    <h2 className="text-2xl font-semibold text-[#111111]">Studenti iscritti</h2>
                    <button
                        type="button"
                        aria-label="Search"
                    ...
                    <table className="w-full min-w-[980px] table-auto border-collapse">
                        <thead>
                            <tr className="border-b border-[#d6d6d6] bg-[#e9e9e9]">
                                <th className="px-10 py-6 text-left text-xl font-semibold text-[#1f1f1f]">Corsista</th>
                                <th className="px-10 py-6 text-left text-xl font-semibold text-[#1f1f1f]">Data di inizio</th>
                                <th className="px-10 py-6 text-left text-xl font-semibold text-[#1f1f1f]">E-mail utilizzata</th>
                                <th className="px-10 py-6 text-left text-xl font-semibold text-[#1f1f1f]">Avanzamento</th>
                                <th className="px-10 py-6 text-left text-xl font-semibold text-[#1f1f1f]">Azioni</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {enrolledStudents.map((student) => (
                                <tr key={student.id} className="border-b border-[#d6d6d6]">
                                    <td className="px-10 py-7 text-lg font-normal text-[#2a2a2a]">{student.name}</td>
                                    <td className="px-10 py-7 text-lg font-normal text-[#2a2a2a]">{student.startDate}</td>
                                    <td className="max-w-[400px] truncate px-10 py-7 text-lg font-normal text-[#2a2a2a]">{student.email}</td>
                                    <td className="px-10 py-7">
                                        <ProgressCell value={student.progress} />
                                    </td>
                                    <td className="px-10 py-7">
                                        <ActionPills type={student.action} onEdit={() => handleOpenPersonalDetails(student)} />
                                    </td>
                                </tr>
                            ))}
                        </tbody>
