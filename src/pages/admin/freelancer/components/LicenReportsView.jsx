...
const LicenReportsView = () => {
    ...
    const getX = (index) => padding.left + (index / (days.length - 1)) * graphWidth;
    const getY = (value) => padding.top + graphHeight - ((value - minY) / (maxY - minY)) * graphHeight;

    const toPoints = (dataset) => dataset.map((value, index) => ({ x: getX(index), y: getY(value) }));
    ...
    return (
        <section className="mt-5 space-y-7">
            <div className="flex items-center justify-between">
                <h1 className="text-[28px] font-bold leading-none text-[#232323] md:text-[32px]">Report & Statistiche</h1>
            </div>

            <div>
                <label className="relative inline-flex items-center">
                    <CalendarDays size={15} className="pointer-events-none absolute left-4 text-[#727272]" />
                    <select
                        value={dateRange}
                        onChange={(e) => setDateRange(e.target.value)}
                        className="h-9 appearance-none rounded-md border border-[#e5e7eb] bg-white pl-10 pr-9 text-[12px] font-medium text-[#5b5b5b] outline-none"
                    >
                    ...
                    <ChevronDown size={14} className="pointer-events-none absolute right-3 text-[#888888]" />
                </label>
            </div>
            ...
            <div className="rounded-xl border border-[#e6e6e6] bg-white px-4 py-5 shadow-[0_1px_1px_rgba(0,0,0,0.02)] md:px-5">
                ...
                <div className="overflow-x-auto">
                    <svg width={chartWidth} height={chartHeight} className="min-w-[860px]" viewBox={`0 0 ${chartWidth} ${chartHeight}`}>
                        {[0, 100, 200, 300, 400, 500, 600, 700].map((val) => {
                            const y = getY(val);
                            return (
                                <g key={`grid-${val}`}>
                                    <line x1={padding.left} y1={y} x2={padding.left + graphWidth} y2={y} stroke="#ececec" strokeWidth={1} />
                                </g>
                            );
                        })}
                        ...
                        <line x1={padding.left} y1={padding.top + graphHeight} x2={padding.left + graphWidth} y2={padding.top + graphHeight} stroke="#d8dfdd" strokeWidth={1.4} />
